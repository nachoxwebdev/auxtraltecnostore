(function() {
    ...
    function renderFeatured() {
        ...
        if (featured.length === 0) {
            container.innerHTML = '<p style="text-align:center;">No hay productos destacados</p>';
            return;
        }
    ...
    // Generar HTML de tarjeta (sin stock visible)
    function createProductCardHtml(p) {
        let avail = getAvailableStock(p.id);
        let imgHtml = p.image ? `<img src="${p.image}" onerror="this.src='https://placehold.co/300x200?text=Sin+Imagen'">` : '<div style="font-size:3rem;">📦</div>';
        ...
        return `
            <div class="product-card" data-id="${p.id}">
                ...
                <button class="product-card__add-btn ${btnDisabled}" data-id="${p.id}" ${btnDisabled}>${inCart ? '✓ En carrito' : '🛒 Agregar'}</button>
            </div>
        `;
    }
    ...
    function renderCartItems() {
        ...
        if (cart.length === 0) {
            container.innerHTML = '<p style="text-align:center;">Carrito vacío</p>';
        ...
        container.innerHTML = cart.map(item => `
            <div class="cart-item">
                <img src="${item.image || ''}" onerror="this.src='https://placehold.co/50x50?text=No'">
                <div style="flex:1"><strong>${item.name}</strong><br>${formatPrice(item.price,item.currency)}</div>
        ...
        $$('.qty-btn').forEach(btn => btn.addEventListener('click', () => {
            ...
            let delta = +btn.dataset.delta;
            let item = cart.find(i => i.id === id);
            if (item) {
                let newQty = item.qty + delta;
                let productStock = products.find(p => p.id === id)?.stock || 0;
                if (newQty < 1) cart = cart.filter(i => i.id !== id);
                else if (newQty <= productStock) item.qty = newQty;
                else { showToast('Stock máximo', true); return; }
                saveCart();
                updateCartUI();
                renderProducts();
                renderFeatured();
    ...
    // Modal de producto (sin mostrar stock)
    function openModal(pid) {
        let p = products.find(p => p.id === pid);
        if (!p) return;
        let flavor = p.flavor ? `<div><strong>Sabor:</strong> ${p.flavor}</div>` : '';
        let imgHtml = p.image ? `<img src="${p.image}" onerror="this.src='https://placehold.co/500x400?text=Sin+Imagen'">` : '<div style="font-size:4rem;">📦</div>';
        let avail = getAvailableStock(pid);
            ...
            <div class="modal-product">
                ...
                <div class="modal-product__info">
                    ...
                    <div style="display:flex; gap:1rem; align-items:center;">
                        <input type="number" id="modalQty" value="1" min="1" max="${avail}" style="width:80px; padding:10px; border-radius:12px; background:#161625; border:1px solid #252538; color:white;">
        ...
        $('#modalAddBtn')?.addEventListener('click', () => {
            ...
            if (qty > avail) { showToast(`Máximo ${avail} unidades`, true); return; }
            let existing = cart.find(i => i.id === pid);
    ...
    // Eventos generales
    function bindEvents() {
        $('#cartToggle').onclick = () => { $('#cartDrawer').classList.toggle('open'); $('#cartOverlay').classList.toggle('visible'); };
        ...
        // Cierre del banner con imagen
        const closeBanner = $('#closeBannerBtn');
        if (closeBanner) {
            closeBanner.onclick = () => {
        ...
        if (localStorage.getItem('banner_closed') === 'true') $('#promoBanner').style.display = 'none';
    }
